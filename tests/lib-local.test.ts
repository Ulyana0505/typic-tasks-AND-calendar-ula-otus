...
describe('lib-local', () => {
    it('createApi', async () => {
        ...
        expect(await api.loadTodo(id)).toEqual(undefined)
        expect(
            (await api.filterTodo({ fieldName: 'content', value: 'aaa' }))
                .length
        ).toEqual(0)
        ...
        expect(
            (await api.filterTodo({ fieldName: 'date', value: date })).length
        ).toEqual(2)
        expect(
            (await api.filterTodo({ fieldName: 'tag', value: '' })).length
        ).toEqual(0)
        expect(
            (
                await api.filterTodo({
                    fieldName: 'status',
                    value: String(todoStatusOptions.active),
                })
            ).length
        ).toEqual(0)
        expect(
            (await api.filterTodo({ fieldName: '', value: '' })).length
        ).toEqual(2)
    })
})
